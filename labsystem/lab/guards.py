...
def _blocking_message(visit_service, order) -> str:
    return (
        f"{visit_service.service.name} already has lab work in progress "
...
def release_visit_service_for_lab(visit_service):
    """
    Call immediately before deleting a single VisitService. If it has a
    LabOrder that never progressed past PENDING, that's safe to retract
    silently (nothing has happened yet) — delete it so PROTECT doesn't
    block a legitimate removal. If the order has moved on (sample
    collected, results entered, ...), raise instead of quietly discarding
    real lab work.
    """
    order = getattr(visit_service, "lab_order_next", None)
    if order is None:
        return
...
def release_visit_services_for_lab(visit_services):
    ...
    voiding a visit). Two passes on purpose: check every service first and
    raise before touching anything if any one of them blocks, only then
    delete the PENDING orders that are actually safe to retract. Doing the
    check-and-delete in one pass would let earlier deletions happen even
    though a later item aborts the whole operation.
    """
    orders_to_delete = []
