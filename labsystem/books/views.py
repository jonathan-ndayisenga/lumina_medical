...
def invoice_edit(request, pk):
    ...
    if invoice.status != Invoice.STATUS_DRAFT:
        messages.error(
            request,
            "Only a draft invoice can be edited. This one has already been issued — void it and raise a new "
            "invoice instead, so the accounting trail stays intact.",
        )
        return redirect("books:invoice_detail", pk=invoice.pk)
...
def invoice_delete(request, pk):
    ...
    if invoice.status != Invoice.STATUS_DRAFT:
        messages.error(
            request,
            "Only a draft invoice can be deleted. This one has already been issued — void it instead, so the "
            "accounting trail stays intact.",
        )
        return redirect("books:invoice_detail", pk=invoice.pk)
    if request.method == "POST":
        label = invoice.number or f"Draft #{invoice.pk}"
        invoice.delete()
        messages.success(request, f"{label} deleted.")
        return redirect("books:invoice_list")
    return render(request, "books/confirm.html", {
        "message": f"Delete draft invoice {invoice.number or f'#{invoice.pk}'}? This cannot be undone.",
        "cancel_url": reverse("books:invoice_detail", args=[invoice.pk]),
    })
...
def invoice_detail(request, pk):
    invoice = get_object_or_404(
        Invoice.objects.select_related("client", "journal_entry").prefetch_related("allocations__payment"),
        pk=pk,
    )
...
@books_staff_required
def payment_create(request, client_pk=None, invoice_pk=None):
    ...
    if request.method == "POST":
        form = PaymentForm(request.POST, client=client, invoice=invoice)
        if form.is_valid():
            payment = form.save(commit=False)
            picked_invoice = form.cleaned_data.get("invoice")
            allocations = [(picked_invoice, min(payment.amount, picked_invoice.balance))] if picked_invoice else None
            try:
        ...
        form = PaymentForm(initial={"date": date.today()}, client=client, invoice=invoice)
    return render(request, "books/payment_form.html", {"form": form, "client": client, "invoice": invoice})
